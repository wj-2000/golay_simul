...
def main():
    ...
    f = 356000 #in μm
    grid = 5.6 #in μm
    wavelength_nominal = 4.500 #in μm

    phasefield = np.load('phase_and_aperture/center_aperture_with_hyperboloid_phase_grid_5.6um.npy')
    aperture = np.load('phase_and_aperture/center_aperture_transmission.npy')
    ...
    #Save 2d psf
    folder_path = os.path.join(os.path.dirname(__file__), f"grid{grid}/psf_center/{optimize_type}")
    os.makedirs(folder_path, exist_ok=True)
    file_path = os.path.join(folder_path, f"{optimize_type}_center_r8_psf_{wavelength.detach().numpy():.3f}_2d.npy")
    ...
    plt.xlabel('x (μm)')
    plt.ylabel('y (μm)')
    plt.savefig(os.path.join(folder_path, f"{optimize_type}_center_r8_psf_{wavelength.detach().numpy():.3f}_2d.png"))
    ...
    plt.savefig(os.path.join(folder_path, f"{optimize_type}_center_r8_psf_{wavelength.detach().numpy():.3f}_1d.png"))
